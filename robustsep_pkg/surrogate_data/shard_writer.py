...
class SurrogateShardWriterConfig:
    ...
    manifest_version: str = "surrogate-shards-v1"
...
def write_surrogate_training_shards(
    ...
    *,
    ...
    for row in _iter_surrogate_rows(generated_records, ppp):
        buffer.append(row)
...
def _iter_surrogate_rows(generated_records: Iterable[Any], ppp: PPP) -> Iterator[_SurrogateRow]:
    for target_record in generated_records:
        for example_index, example in enumerate(target_record.surrogate_examples):
            ...
            metadata = {
                ...
                "lambda_value": example.lambda_value,
