...
\title{RobustSep: A Non-Deterministic Architecture for Robust, Drift-Aware Color Separation in Packaging}
\author{
...
The memory footprint of high-resolution 3D LUTs (approximately 32 MB for an 8-bit LUT) motivated early use of neural networks as compact approximators. Marcu and Iwata \cite{marcu} trained a 3-12-12-12-4 multilayer perceptron on synthetic CMYK to RGB forward mappings computed via the Neugebauer equations with Yule-Nielsen corrections, demonstrating that a learned model could approximate the highly nonlinear inverse mapping. Subsequent work by ZhiChuan et al.\ \cite{icc} using a cerebellar model neural network (CMAC) achieved a mean $\Delta E$ of 1.6 on CMYK-to-Lab conversion; Zhang and colleagues \cite{kingma} reduced mean $\Delta E$ by approximately 23\% using polynomial regression and basic neural networks. These approaches confirmed the viability of data-driven colour-space inversion but shared a limitation: all learned weights encoded a globally fixed GCR policy, with no sensitivity to the spatial frequency, tonal character, or semantic content of specific image regions.
...
Given an RGB patch $x_u$, the objective is to recover an ink separation $y_u$ that lies within a feasible set defined by a Press Prior Package (PPP). This feasible set enforces hard constraints such as per-channel ink limits and a maximum total area coverage (TAC), ensuring that all admissible solutions are physically printable.
...
While this formulation captures variability in the printing process, it implicitly assumes that all perceptual deviations are equally consequential across the image, which is not the case in practice. In commercial print, all color errors are assumed to carry equal perceptual weight \cite{zeng}. A $\Delta E_{00}$ deviation on a brand mark is likely to trigger a press stop although the same deviation in a background gradient field may be unnoticed. Standard perceptual objectives are indifferent to this distinction. We address this by assigning each pixel to one of three intent classes, derived directly from the input RGB image through local statistics and edge analysis, with no learned components. Brand pixels cover logos, wordmarks, and hue-specified spot-color regions where colorimetric accuracy is a contractual requirement. Flat pixels cover uniform or near-uniform fields where tonal stability and absence of banding matter most. Gradient pixels cover smooth tonal transitions like vignettes, continuous-tone imagery, skin tones where the primary failure mode is contouring rather than mean color error. Classification is resolved in strict priority order (Brand $>$ Gradient $>$ Flat), with unclassified pixels defaulting to Flat. Per-patch intent weights $(w_u^B, w_u^F, w_u^G)$ are the normalized pixel-class fractions for patch $u$. These enter the pipeline through two complementary paths: as constant per-pixel channels concatenated to the proposer's first-layer input (making intent spatially visible at the input level), and as a coarse $4\times 4$ intent raster processed through a dedicated intent embedding MLP. Both representations fold into $c_u$ and propagate via FiLM into every subsequent network block.
...
The proposer is queried three times per patch, once per $\lambda$ value $\{0.1, 0.5, 0.9\}$, each with a distinct deterministic seed. A hinge penalty during training enforces that mean OGV ink usage is non-decreasing across these three $\lambda$ levels, so expanded-gamut ink usage scales predictably with the operator's intent.
...
\]
where $\hat{A} = T_{\text{ICC}}(\hat{Y})$ and $A^* = T_{\text{ICC}}(Y^*)$ are the ICC-rendered Lab appearances of the predicted and target patches, respectively, under Relative Colorimetric intent with Black Point Compensation \cite{marcu}. The weight $\alpha_{\text{app}} \ll 1$ keeps this term a regularizer rather than a primary signal.

The forward surrogate is a lightweight Micro-CNN whose purpose is not artistic fidelity but computationally efficient, differentiable approximation of ICC-based print rendering for candidate ranking and robustness gating. It takes a $32\times 32$ CMYKOGV context window as input (providing spatial neighborhood context for halftone ink interactions and overprint effects), but is scored only on the inner $16\times 16$ region \cite{kingma}. It comprises eight convolutional blocks with channel progression $32 \rightarrow 32 \rightarrow 64 \rightarrow 64 \rightarrow 96 \rightarrow 96 \rightarrow 64 \rightarrow 32$. Block 0 is plain Conv$\rightarrow$GroupNorm$\rightarrow$SiLU with no FiLM; Blocks 1--7 follow Conv$\rightarrow$GroupNorm$\rightarrow$FiLM$\rightarrow$SiLU. Dilated convolutions (dilation rates 1, 2, 2, 4, 4, 1, 1 for Blocks 1--7) enlarge the effective receptive field without reducing spatial resolution. A $1\times 1$ linear head outputs the three-channel $(L,a,b)$ prediction.
...
The surrogate is trained entirely under ICC teacher supervision \cite{marcu}. For each training example, two ICC teacher targets are generated: a nominal target (no drift) and a drifted target (one PPP-sampled drift instance applied before ICC rendering). Both are supervised symmetrically using Huber loss with $\delta = 1.0$ in Lab space:
\[
L_{\text{nominal}} =
\frac{1}{HWC}\sum_{i,j,c}
\operatorname{Huber}_{\delta}\!\bigl(\hat{L}^{(n)}_{i,j,c} - L^{(n)}_{i,j,c}\bigr),
\]
\[
L_{\text{drift}} =
\frac{1}{HWC}\sum_{i,j,c}
\operatorname{Huber}_{\delta}\!\bigl(\hat{L}^{(d)}_{i,j,c} - L^{(d)}_{i,j,c}\bigr),
\]
\[
...
Every proposed candidate passes through a deterministic two-pass refiner before entering robustness evaluation. Pass 1 applies spatial regularity smoothing and neutral/dark stabilization to suppress high-frequency separation artifacts and ink-balance violations in perceptually sensitive regions. Pass 2 applies the feasibility projection operator $\Pi_K$:
\[
Y'_c \leftarrow \operatorname{clip}(Y_c, 0, \kappa_c),
\]
\[
Y \leftarrow Y' \cdot \min\!\left(1, \frac{TAC_{\max} - \varepsilon}{\sum_c Y'_c}\right).
\]
The small margin $\varepsilon$ prevents numerical instability at the exact TAC boundary. The composed output
\[
\tilde{Y}_k = \Pi_K\bigl(R_1(Y_k)\bigr)
\]
is guaranteed to lie inside $K(\mathrm{PPP})$. Projection is applied after every gradient step during target generation and unconditionally at inference. Candidates requiring large projection corrections tend to accumulate higher robustness risk scores and are deprioritized during selection.
...
\]
The surrogate predicts CIELab appearance
\[
\widehat{LAB}_{k,i} = F_{\theta}(\tilde{Y}_{k,i}, c_u)
\]
...
We evaluate the proposed RobustSep pipeline on a dataset of 1,000 RGB images sampled from \underline{\hspace{3cm}}. The images are resized and processed in a patch-wise manner using the proposed Propose--Refine--Evaluate--Select framework.

All experiments are conducted using pretrained models for the Conditional VAE (Proposer) and the Micro-CNN surrogate (forward rendering approximation).

The evaluation focuses on three key aspects: color fidelity, structural preservation, and print feasibility.

\subsection{Evaluation Metrics}

We adopt a comprehensive set of metrics inspired by prior works such as Chen et al.\ and Zeng et al.\ to ensure a rigorous comparison.

\begin{itemize}
    \item \textbf{Color Fidelity:} Evaluated using CIE $\Delta E$, Mean Squared Error (MSE), Peak Signal-to-Noise Ratio (PSNR), and Structural Similarity Index (SSIM).
    \item \textbf{Structure Preservation:} Measured mathematically by the Edge Error (\%), evaluating whether fine text and borders are maintained.
    \item \textbf{Print Feasibility:} Quantified practically via Total Area Coverage (TAC), Ink Consumption per channel, Neutral Gray Balance stability, and K-channel distribution statistics (mean, std).
\end{itemize}

\subsection{Quantitative Results}

The quantitative performance of RobustSep compared to standard baseline approaches is presented in Table~\ref{tab:quant_results}. Furthermore, Table~\ref{tab:percentile_robustness} details the robustness characteristics at the 50th, 75th, and 90th percentiles to demonstrate the system's resilience to tail-risk drift events.
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.8\textwidth}{\vspace{2em}\centering INSERT IMAGE GRID HERE\\Layout: Original RGB $\mid$ Predicted CMYK $\rightarrow$ RGB $\mid$ Error Map\vspace{2em}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.65\textwidth}{\vspace{2em}\centering INSERT HISTOGRAM\vspace{2em}}}
\caption{$\Delta E$ Perceptual Error Distribution. Most samples fall within [INSERT RANGE]. The tail distribution characterizes our model's worst-case failure behavior.}
\label{fig:de_dist}
\end{figure}
...
\centering
\fbox{\parbox{0.65\textwidth}{\vspace{2em}\centering INSERT HISTOGRAM\vspace{2em}}}
\caption{TAC Distribution across the evaluation dataset. Expected Mean TAC: [INSERT], Percent Exceeding Threshold Limit: [INSERT].}
\label{fig:tac_dist}
\end{figure}

\begin{figure}[htbp]
\centering
\fbox{\parbox{0.65\textwidth}{\vspace{2em}\centering INSERT HISTOGRAM / KDE\vspace{2em}}}
...
We perform an ablation study to isolate the impact of our core architectural components: the deterministic refiner, the forward surrogate, and the PPP constraints. The results are summarized in Table~\ref{tab:ablation}.
...
\begin{tabular}{lccc}
\toprule
Variant & $\Delta E$ & TAC & Edge Error \\
\midrule
Full Model & [ ] & [ ] & [ ] \\
w/o Refiner & [ ] & [ ] & [ ] \\
w/o Surrogate & [ ] & [ ] & [ ] \\
...
The comprehensive evaluations across color fidelity, structural preservation, and print constraint satisfaction clearly demarcate the advantages of the RobustSep pipeline. [PENDING TEXT...]
