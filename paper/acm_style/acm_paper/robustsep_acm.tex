...
\title[RobustSep]{RobustSep: A Stochastic Candidate-Generation Architecture for Robust, Drift-Aware Color Separation in Packaging}

\author{Shreeya Srivastava}
\authornote{Equal contribution.}
\affiliation{%
  \institution{Constrained Image-Synthesis Lab}
  \city{Mandi}
  \country{India}}
...
\author{Rushabh Lodha}
\affiliation{%
  \institution{IIT Mandi}
  \city{Mandi}
  \country{India}}

\renewcommand{\shortauthors}{Srivastava et al.}
...
\keywords{RGB-to-CMYK separation, packaging color management, robust optimization, conditional variational autoencoder, process drift}

\begin{teaserfigure}
\centering
\includegraphics[width=\textwidth]{../../figures/robustsep_teaser.png}
\Description{A qualitative teaser panel comparing RGB source appearance with separation variants and RobustSep output.}
\caption{Qualitative teaser comparing source RGB appearance with unconstrained, feasibility-projected, and RobustSep separations. The panel illustrates the intended behavior of preserving visual fidelity while enforcing print feasibility and drift-aware robustness.}
\label{fig:teaser}
\end{teaserfigure}
...
\[
\begin{aligned}
\text{minimise}\quad & \sum (C + M + Y + K) \\
\text{subject to}\quad & K \leq K_{\text{target}}, \\
& \sum (C + M + Y + K) \leq TAC_{\max}.
\end{aligned}
\]
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/robustsep_system_manifold_nocaption.png}
\Description{A left-to-right operator pipeline showing RGB patch conditioning, stochastic proposal, feasibility projection, drift-risk evaluation, and final candidate selection.}
...
\Description{A feasibility diagram showing PPP channel caps, TAC, OGV limits, pair constraints, and projection into the feasible set.}
...
\Description{A proposal-manifold diagram showing latent samples and lambda conditioning producing alternative CMYKOGV candidate separations for a fixed patch.}
...
The encoder comprises five convolutional blocks E0--E4. E0 is a plain Conv, GroupNorm, SiLU block with no conditioning. E1 and E3 use strided convolutions (stride 2) to progressively reduce spatial resolution from $16\times 16$ to $4\times 4$; E2 and E4 are stride-1 refinement blocks. All of E1--E4 follow the sequence Conv, GroupNorm, FiLM, SiLU. Global average pooling at $4\times 4$ produces a compact feature vector, projected by two linear heads to the latent mean $\mu$ and log-variance $\log \sigma^2$.

The decoder D0--D5 reverses the hierarchy. D0 linearly projects $z$ to a $96\times 4\times 4$ tensor with no FiLM. D1--D5 are FiLM-conditioned Conv, GroupNorm, FiLM, SiLU blocks with bilinear $2\times$ upsampling at D2 and D4, restoring $16\times 16$ resolution. A final $1\times 1$ sigmoid head produces the 7-channel output. FiLM scale is applied as $1 + \gamma_i$ to maintain near-identity initialization stability. Each FiLM-enabled block carries its own two-layer MLP head (Linear, SiLU, Linear) mapping $c_u$ to per-channel scale and shift vectors; heads are not shared across blocks.
...
\Description{A drift-risk selection diagram showing multiple feasible candidates evaluated over deterministic drift samples, scored by Delta E errors, and selected by 90th-percentile risk.}
...
Qualitative visual comparisons are exported from the evaluation suite as artifact-linked panels. Each panel juxtaposes the original RGB target appearance, the rendered separation, and a $\Delta E_{00}$ error map computed under the declared PPP and drift protocol. These panels are included in the reproducibility bundle rather than represented here as empty draft placeholders.

\subsection{Distribution Analysis}

To better understand the variance and tail events produced under print drift, we analyze the distribution of key separation metrics across the entire test set. The evaluation suite emits three primary distribution artifacts: a $\Delta E_{00}$ histogram emphasizing the right tail under PPP-sampled drift, a TAC histogram reporting mass near and above the PPP threshold, and a K-channel histogram or KDE summarizing black-generation behavior across separation candidates.
